...
pub trait GateInstructions<F: ScalarField> {
    ...
    fn load_witness(&self, ctx: &mut Context<'_, F>, witness: Value<F>) -> AssignedValue<F> {
        self.assign_region_last(ctx, [Witness(witness)], [])
    }

    fn load_constant(&self, ctx: &mut Context<'_, F>, c: F) -> AssignedValue<F> {
    ...
    // | a | -a | 1 | 0 |
    fn neg(&self, ctx: &mut Context<'_, F>, a: QuantumCell<'_, F>) -> AssignedValue<F> {
    ...
    fn not(&self, ctx: &mut Context<'_, F>, a: QuantumCell<'_, F>) -> AssignedValue<F> {
        ...
        let ind = self.assign_region(
            ...
            (0..len).map(|i| {
                Witness(idx.value().map(|x| {
                    if u64::from_le_bytes(x.to_repr()[0..8].try_into().unwrap()) == i as u64 {
                        F::ONE
                    } else {
                        F::ZERO
                    }
    ...
    fn is_zero(&self, ctx: &mut Context<'_, F>, a: &AssignedValue<F>) -> AssignedValue<F> {
        let (is_zero, inv) = a
            .value()
            .map(|x| {
