...
pub fn assign<F: FieldExt>(
	...
	if idx == 0 {
	    carries.push(a_val / limb_val.clone());
	} else {
	    let carry_val = carries[idx - 1].clone();
	    carries.push((a_val + carry_val) / limb_val.clone());
	}
    }
	...
	|mut region| {
		...
		let last_carry = region.assign_advice(
		    ...
		    range.qap_config.value,
		    offset + 1,
		    || Ok(big_to_fe::<F>(modulus::<F>() - carries[idx].clone()))
		)?;
		...
		let limb = region.assign_advice_from_constant(
		    ...
		    range.qap_config.value,
		    offset + 2,
		    big_to_fe::<F>(limb_val.clone())
		)?;
		region.constrain_constant(limb.cell(), big_to_fe::<F>(limb_val.clone()))?;
    ...
    let range_bits = (((max_limb_bits - (limb_bits as u64) + 1 + (range.lookup_bits as u64) - 1) / (range.lookup_bits as u64)) * (range.lookup_bits as u64)) as usize;
    let shift_val = big_to_fe::<F>(BigUint::from(1u32) << (range_bits - 1));
