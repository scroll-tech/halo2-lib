...
use crate::{
    ...
    fields::{fp::FpConfig, fp12::mul_no_carry_w6, fp2, FieldChip, FieldExtPoint},
    gates::{GateInstructions, QuantumCell::*},
    utils::{
...
impl<'a, 'b, F: FieldExt> Fp12Chip<'a, 'b, F> {
    ...
    pub fn cyclotomic_decompress(
        ...
        let g5_sq = fp2_chip.mul_no_carry(layouter, &g5, &g5)?;
        let g5_sq_c = mul_no_carry_w6::<F, FpChip<F>, XI_0>(fp2_chip.fp_chip, layouter, &g5_sq)?;

        let g4_sq = fp2_chip.mul_no_carry(layouter, &g4, &g4)?;
        let g4_sq_3 = fp2_chip.scalar_mul_no_carry(layouter, &g4_sq, F::from(3))?;
        let g3_2 = fp2_chip.scalar_mul_no_carry(layouter, &g3, F::from(2))?;

        let mut g1_num = fp2_chip.add_no_carry(layouter, &g5_sq_c, &g4_sq_3)?;
        g1_num = fp2_chip.sub_no_carry(layouter, &g1_num, &g3_2)?;
        // can divide without carrying g1_num or g1_denom (I think)
        let g2_4 = fp2_chip.scalar_mul_no_carry(layouter, &g2, F::from(4))?;
        let g1_1 = fp2_chip.divide(layouter, &g1_num, &g2_4)?;

        let g4_g5 = fp2_chip.mul_no_carry(layouter, &g4, &g5)?;
        let g1_num = fp2_chip.scalar_mul_no_carry(layouter, &g4_g5, F::from(2))?;
        let g1_0 = fp2_chip.divide(layouter, &g1_num, &g3)?;

        let g2_is_zero = fp2_chip.is_zero(layouter, &g2)?;
        // resulting `g1` is already in "carried" format (witness is in `[0, p)`)
        let g1 = fp2_chip.select(layouter, &g1_0, &g1_1, &g2_is_zero)?;

        // share the computation of 2 g1^2 between the two cases
        let g1_sq = fp2_chip.mul_no_carry(layouter, &g1, &g1)?;
        let g1_sq_2 = fp2_chip.scalar_mul_no_carry(layouter, &g1_sq, F::from(2))?;

        let g2_g5 = fp2_chip.mul_no_carry(layouter, &g2, &g5)?;
        let g3_g4 = fp2_chip.mul_no_carry(layouter, &g3, &g4)?;
        let g3_g4_3 = fp2_chip.scalar_mul_no_carry(layouter, &g3_g4, F::from(3))?;
        let temp = fp2_chip.add_no_carry(layouter, &g1_sq_2, &g2_g5)?;
        let temp = fp2_chip.select(layouter, &g1_sq_2, &temp, &g2_is_zero)?;
        let temp = fp2_chip.sub_no_carry(layouter, &temp, &g3_g4_3)?;
        let mut g0 = mul_no_carry_w6::<F, FpChip<F>, XI_0>(fp2_chip.fp_chip, layouter, &temp)?;
        ...
        // finally, carry g0
        g0 = fp2_chip.carry_mod(layouter, &g0)?;
    ...
    pub fn cyclotomic_square(
        ...
        let g2_plus_g3 = fp2_chip.add_no_carry(layouter, g2, g3)?;
        let cg3 = mul_no_carry_w6::<F, FpChip<F>, XI_0>(fp2_chip.fp_chip, layouter, g3)?;
        let g2_plus_cg3 = fp2_chip.add_no_carry(layouter, &g2, &cg3)?;
        let a23 = fp2_chip.mul_no_carry(layouter, &g2_plus_g3, &g2_plus_cg3)?;

        let g4_plus_g5 = fp2_chip.add_no_carry(layouter, g4, g5)?;
        let cg5 = mul_no_carry_w6::<F, FpChip<F>, XI_0>(fp2_chip.fp_chip, layouter, g5)?;
        let g4_plus_cg5 = fp2_chip.add_no_carry(layouter, &g4, &cg5)?;
        let a45 = fp2_chip.mul_no_carry(layouter, &g4_plus_g5, &g4_plus_cg5)?;

        let b23 = fp2_chip.mul_no_carry(layouter, &g2, &g3)?;
        let b45 = fp2_chip.mul_no_carry(layouter, &g4, &g5)?;
        let b45_c = mul_no_carry_w6::<F, FpChip<F>, XI_0>(fp2_chip.fp_chip, layouter, &b45)?;

        let mut temp = fp2_chip.scalar_mul_and_add_no_carry(layouter, &b45_c, g2, F::from(3))?;
        let h2 = fp2_chip.scalar_mul_no_carry(layouter, &temp, F::from(2))?;

        temp = fp2_chip.add_no_carry(layouter, &b45_c, &b45)?;
        temp = fp2_chip.sub_no_carry(layouter, &a45, &temp)?;
        temp = fp2_chip.scalar_mul_no_carry(layouter, &temp, F::from(3))?;
        let h3 = fp2_chip.scalar_mul_and_add_no_carry(layouter, &g3, &temp, -F::from(2))?;

        const XI0_PLUS_1: u64 = XI_0 + 1;
        // (c + 1) = (XI_0 + 1) + u
        temp = mul_no_carry_w6::<F, FpChip<F>, XI0_PLUS_1>(fp2_chip.fp_chip, layouter, &b23)?;
        temp = fp2_chip.sub_no_carry(layouter, &a23, &temp)?;
        temp = fp2_chip.scalar_mul_no_carry(layouter, &temp, F::from(3))?;
        let h4 = fp2_chip.scalar_mul_and_add_no_carry(layouter, &g4, &temp, -F::from(2))?;

        temp = fp2_chip.scalar_mul_and_add_no_carry(layouter, &b23, g5, F::from(3))?;
        let h5 = fp2_chip.scalar_mul_no_carry(layouter, &temp, F::from(2))?;

        Ok([h2, h3, h4, h5]
            .iter()
            .map(|h| fp2_chip.carry_mod(layouter, h).expect("carry mod should not fail"))
            .collect())
    }
    ...
    pub fn hard_part_BN(
        ...
        // m^x
        let mx = self.cyclotomic_pow(layouter, m, vec![BN_X])?;
        // m^{x^2}
        let mx2 = self.cyclotomic_pow(layouter, &mx, vec![BN_X])?;
        // m^{x^3}
        let mx3 = self.cyclotomic_pow(layouter, &mx2, vec![BN_X])?;
