...
impl<F: FieldExt> FieldChip<F> for Fp12Chip<F> {
	...
	let mut a1b1_coeffs = Vec::with_capacity(36);
		...
		let coeff01 = self.fp_chip.mul_no_carry(layouter, &a.coeffs[i], &b.coeffs[j + 6])?;
		let coeff10 = self.fp_chip.mul_no_carry(layouter, &a.coeffs[i + 6], &b.coeffs[j])?;
		let coeff11 = self.fp_chip.mul_no_carry(layouter, &a.coeffs[i + 6], &b.coeffs[j + 6])?;
	...
	for i in 0..11 {
	    let a0b0_minus_a1b1_entry = self.fp_chip.sub_no_carry(layouter, &a0b0_coeffs[i], &a1b1_coeffs[i])?;
	    let a0b1_plus_a1b0_entry = self.fp_chip.add_no_carry(layouter, &a0b1_coeffs[i], &a1b0_coeffs[i])?;
	...
	for i in 0..6 {
	    let coeff1 = self.fp_chip.sub_no_carry(layouter, &a0b0_minus_a1b1[i], &a0b1_plus_a1b0[i + 6])?;
	    let coeff2 = self.fp_chip.scalar_mul_no_carry(layouter, &a0b0_minus_a1b1[i + 6], F::from(9))?;
	...
	for i in 0..6 {
	    let coeff1 = self.fp_chip.add_no_carry(layouter, &a0b1_plus_a1b0[i], &a0b0_minus_a1b1[i + 6])?;
	    let coeff2 = self.fp_chip.scalar_mul_no_carry(layouter, &a0b1_plus_a1b0[i + 6], F::from(9))?;
	    let coeff = self.fp_chip.add_no_carry(layouter, &coeff1, &coeff2)?;
	    if i < 5 {
		out_coeffs.push(coeff);
	    } else {
