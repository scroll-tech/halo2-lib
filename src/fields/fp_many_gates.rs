...
impl<F: FieldExt, const K: usize> FpChip<F, K> {
    ...
    pub fn configure(meta: &mut ConstraintSystem<F>) -> FpConfig<F, K> {
        ...
        for x in 0..(2 * K - 1) as u32 {
            meta.create_gate("BigInt Lagrange", |meta| {
                ...
                let a_vec: Vec<Expression<F>> = Vec::from_iter(0..K as u32)
                    .iter()
                    .map(|&idx| meta.query_advice(a, Rotation(idx.try_into().unwrap())))
                    .collect();

                let b_vec: Vec<Expression<F>> = Vec::from_iter(0..K as u32)
                    .iter()
                    .map(|&idx| {
                ...
                let c_vec: Vec<Expression<F>> = Vec::from_iter(0..(2 * K - 1) as u32)
                    .iter()
                    .map(|&idx| {
                ...
                let x_vec: Vec<Expression<F>> = Vec::from_iter(0..(2 * K - 1) as u32)
                    .iter()
                    .map(|&idx| Expression::Constant(F::from(u32::pow(x, idx) as u64)))
                    .collect();
    ...
    pub fn assign_inputs(
        ...
        layouter.assign_region(
            ...
            |mut region| {
                ...
                let a_temp: Result<Vec<AssignedCell<F, F>>, Error> =
                    a_assigned.into_iter().collect();

                let b_assigned = indices.iter().map(|&idx| {
                    region.assign_advice(|| "inputs", self.config.a, idx + K, || Ok(b_vals[idx]))
                });
                let b_temp: Result<Vec<AssignedCell<F, F>>, Error> =
                    b_assigned.into_iter().collect();
...
impl<F: FieldExt, const K: usize> Circuit<F> for TestCircuit<F, { K }> {
    ...
    fn synthesize(
        ...
        let (a_assigned, b_assigned) = chip.assign_inputs(
            layouter.namespace(|| "inputs"),
            self.a_vals.as_slice(),
            self.b_vals.as_slice(),
        )?;
        chip.assign(
            layouter.namespace(|| "actual circuit"),
            a_assigned,
            b_assigned,
        )?;
        Ok(())
