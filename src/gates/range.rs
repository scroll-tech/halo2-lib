	...
	let padded_bits = k * self.lookup_bits;
		...
		let mut limb = shift_val.as_ref().map(|x| x.modpow(&BigUint::from(1u64), &mask));
                ...
                region.assign_advice(
                    ...
                    self.qap_config.value,
                    offset + 8,
                    || limb.as_ref().map(|x| biguint_to_fe(x)).ok_or(Error::Synthesis),
                )?;
		...
		offset = offset + 9;
                ...
                for idx in 1..(k + 1) {
                    ...
                    let out_cell = region.assign_advice(
                        ...
                        self.qap_config.value,
                        offset + 2,
                        || running_sum.as_ref().map(|sum| biguint_to_fe(sum)).ok_or(Error::Synthesis),
                    )?;		
			...
			let is_zero = limb_cell.value().zip(Some(F::from(1)))
			    .map(|(x, y)| {
			...
			let inv = limb_cell.value().zip(Some(F::from(1)))
			    .map(|(x, y)| {
				if *x == F::from(0) {
				    F::from(1)
				} else {
